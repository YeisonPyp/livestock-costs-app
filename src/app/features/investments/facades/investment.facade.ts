...
export class InvestmentFacade {
  ...
  loadInitialData(): void {
    ...
    forkJoin({
      stats: this.investmentSvc
        .getGlobalSummary()
        .pipe(catchError(() => of(null))),
      investments: this.investmentSvc
        .list({ page: 1, pageSize: this.listFilters().pageSize })
        .pipe(catchError(() => of(null))),
    })
  ...
  loadInvestments(): void {
    ...
    this.investmentSvc
      ...
      .subscribe({
        ...
        error: () => {
          this.listError.set(true);
          this.notify.error('Error al cargar las inversiones');
        },
      });
  }
  ...
  refreshStats(): void {
    ...
    this.investmentSvc
      .getGlobalSummary()
      .subscribe({
        next: (res) => this.applyStats(res.data),
        error: () => this.listStats.update((s) => ({ ...s, isLoading: false })),
      });
  }
  ...
  readonly isActive = computed(
    () => this.detail()?.status === InvestmentStatus.ACTIVE,
  );
  ...
  readonly canWithdraw = computed(() => {
    ...
    return (
      ...
      parseDecimal(inv.currentCapital) > 0
    );
  });
  ...
  loadDetail(id: string): void {
    ...
    forkJoin({
      investment: this.investmentSvc
        .getById(id)
        .pipe(catchError(() => of(null))),
      dashboard: this.investmentSvc
        .getDashboard(id)
        .pipe(catchError(() => of(null))),
    })
  ...
  loadMovements(): void {
    ...
    this.investmentSvc
      .getMovements(id, this.movementFilters())
  ...
  openPanel(type: OperationPanelType): void {
    if (type === 'contribute' && !this.canContribute()) {
      this.notify.warning('No se pueden realizar aportes a esta inversión');
      return;
    }
    if (type === 'withdraw' && !this.canWithdraw()) {
      this.notify.warning('No hay saldo disponible para retirar');
      return;
    }
  ...
  submitWithdraw(payload: WithdrawalPayload): void {
    ...
    this.investmentSvc
      ...
      .subscribe({
        next: () => {
          this.notify.success('Retiro procesado');
          this.closePanel();
          this.refreshDetail();
        },
        error: (err) => this.notify.error(this.extractError(err)),
  ...
  openCloseModal(): void {
    if (!this.canClose()) {
      this.notify.warning('Esta inversión no puede cerrarse aún');
      return;
    }
  ...
  confirmClose(): void {
    ...
    this.investmentSvc
      ...
      .subscribe({
        next: () => {
          this.notify.success('Inversión cerrada');
  ...
  goBackToList(): void {
    this.router.navigate(['/investments/investments']);
  }
  ...
  loadActiveInvestors(): void {
    ...
    this.investorSvc
      ...
      .subscribe({
        ...
        error: () =>
          this.formInvestorsError.set('Error al cargar los inversionistas'),
      });
  }
  ...
  checkActiveContract(investorId: string): void {
    ...
    this.contractSvc
      .list({ investorId, status: ContractStatus.ACTIVE })
  ...
  private applyStats(data: GlobalInvestmentSummary): void {
    this.listStats.set({
      ...
      totalCurrentCapital: data.totalCurrentCapital,
      totalProfits: data.totalProfits,
      totalCosts: data.totalCosts,
      totalClosedCapital: data.totalClosedCapital,
      avgRoi: data.avgRoi,
      isLoading: false,
    });
  }
